<template>
  <div class="news-detail">
    ...
    <!-- 加载中 -->
    <van-loading v-if="newsStore.detailLoading" class="loading-wrapper" size="48" vertical>
      加载中...
    </van-loading>

    <!-- 加载失败 -->
    <van-empty
      v-else-if="newsStore.detailError"
      image="error"
      description="加载失败，请重试"
    >
      <van-button type="primary" @click="retryLoad" size="small">
        重新加载
      </van-button>
    </van-empty>

    <!-- 新闻详情内容 -->
    <div class="detail-content" v-else-if="newsStore.newsDetail.id">
...
// 重试加载
const retryLoad = () => {
  newsStore.getNewsDetail(newsId.value)
}
